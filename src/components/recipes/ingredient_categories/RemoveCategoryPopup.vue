<template>
    <div class="remove-category-popup__overlay">
        <TheWindow>
            <div class="remove-category-popup__content">
                <p>Remove category <b>{{ category.category }}</b>?</p>
                <p v-if="subCategoryCount > 0" class="remove-category-popup__warning">
...
<script setup lang="ts">
    ...
    function countDescendants(cat: RecipeIngredientCategory): number {
        return (cat.subCategories ?? []).reduce((sum, sub) => sum + 1 + countDescendants(sub), 0)
    }

    async function handleConfirm() {
        await (new APICall(`recipe-ingredient-categories/${props.category.id}`, 'DELETE')).execute();
        emit('removed-category');
    }
</script>

<style lang="scss">
    .remove-category-popup__overlay {
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
    ...
    .remove-category-popup__warning {
    ...
    .remove-category-popup__buttons .the-button {
        padding: 4px 12px;
    }
</style>
